...
void mondapp::rundapp ( const account_name subscriber ) {
   
   print ("Before subscription check");
...
void mondapp::apply(const account_name contract, const account_name act)
{
    if (act == N(transfer))
    {
    ...
    switch (act)
    {
        EOSIO_API(mondapp, (rundapp))
    };
}

void mondapp::transferReceived(const currency::transfer &transfer, const account_name code)
{
